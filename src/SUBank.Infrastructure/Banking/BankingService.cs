    ...
    IRealtimeNotifier realtimeNotifier) : IBankingService
{
    ...
    public async Task<TransferResponse> TransferAsync(string userId, string idempotencyKey, TransferRequest request, CancellationToken cancellationToken)
    {
        ...
        var replay = await IdempotencyReplay.FindMatchingAsync(
            dbContext, userId, idempotencyKey, requestHash, TransactionType.Transfer, cancellationToken);
        if (replay is not null)
        {
            return ToTransferResponse(replay, replayed: true);
        }
        ...
        {
            source = await dbContext.BankAccounts.Include(x => x.CustomerProfile)
            ...
            destination = await dbContext.BankAccounts.Include(x => x.CustomerProfile)
            ...
            BankingRules.ValidateCreditedBalance(destination.Balance, request.Amount);
        ...
        if (persistenceFailure is not null)
        {
            ...
            {
                return ToTransferResponse(concurrentReplay, replayed: true);
            }
    ...
    private static TransferResponse ToTransferResponse(FinancialTransaction transaction, bool replayed)
    {
        if (transaction.SourceAccount is null)
            throw new ConflictException("Bản ghi idempotency không phải là giao dịch chuyển tiền hợp lệ.");
