...
public sealed class QrPayloadRulesTests
{
    ...
    {
        var payload = QrPayloadRules.Create("1000000001", 1234m, "  Thanh toán demo  ");

        var result = QrPayloadRules.Parse(payload);

        Assert.Equal("1000000001", result.AccountNumber);
        Assert.Equal(1234m, result.Amount);
    ...
    [InlineData("subank://transfer?v=1&account=1000000001&amount=1.50")]
